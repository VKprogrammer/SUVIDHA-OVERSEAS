...
export async function POST(req: Request) {
  try {
    ...
    const { full_name, email, preferred_country, academic_background } = data;

    const connection = await mysql.createConnection(dbConfig);

    const query = `
      INSERT INTO university_course_requests 
        (full_name, email, preferred_country, academic_background) 
      VALUES (?, ?, ?, ?)
    `;
